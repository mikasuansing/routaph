...
export function PinPickerSheet({ title, initial, onConfirm, onCancel }: Props) {
  ...
  // Shared by the initial locate and the "Current" button: MapLibre takes
  // [lng, lat] and flies with a pitch; Leaflet takes [lat, lng] and has no
  // camera to speak of.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const flyMapTo = (m: any, here: [number, number]) => {
    if (!m) return;
  ...
  useEffect(() => {
    ...
    // No explicit starting point (opened via "Drop a pin on the map" rather
    // than from a known stop) — center on the rider's live position instead
    // of leaving them looking at all of Metro Manila zoomed out. A fresh
    // fix only: maximumAge 0 so this can't hand back a cached fix from
    // earlier in the day, or an old session, instead of where they are now.
    if (!initial && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        pos => {
          if (cancelled) return;
          flyMapTo(mapRef.current, [pos.coords.latitude, pos.coords.longitude]);
        },
        () => { /* no fix — stays on the Metro Manila fallback view */ },
